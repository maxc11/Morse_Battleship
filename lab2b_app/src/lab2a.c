...
static QState game_State_enemy(Lab2A *me);
...
static QState game_State(Lab2A *me) {
    switch (Q_SIG(me)) {
        ...
        case BTN_SIG: {
                   switch (button) {
                       case UP:
                           // Player's turn: check hit on enemy board
                           check_hit(enemy_board,0);

                           // Check if the player has won
                           if (check_winner(enemy_board)) {
                               xil_printf("You sank all enemy ships! You win!\r\n");
                               exit(0); // End the game
                           }
                           ...
                           if (check_winner(user_board)) {
                               xil_printf("Enemy sank all your ships. You lose!\r\n");
                               exit(0); // End the game
                           }

                           // Print the user's board to show the result of the enemy's shot
                           xil_printf("\nUSER BOARD\r\n");
                           print_board();
                       ...
                       case MIDDLE:
                           // Transition to coordinate selector for player's turn
                           return Q_TRAN(&cordinate_Selector);

                       case DOWN:
                           // Print the enemy board for debugging or reference
                           print_enemy_board();
                           break;
...
static QState game_State_enemy(Lab2A *me) {
    switch (Q_SIG(me)) {
        ...
        case BTN_SIG: {
        	switch (button) {
        		case UP:
        			check_hit(user_board[BOARD_SIZE_X][BOARD_SIZE_Y]);
            	break;
